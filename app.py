...
def should_perform_search(prompt):
    ...
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",  # Changed to a faster model for this check
...
def generate_response(prompt, user_id):
    ...
    new_ai_msg = Conversation(user_id=user_id, content=full_response, role="assistant")
...
def get_bot_response():
    ...
    def generate():
        for chunk in generate_response(user_text, user_id):
            yield chunk
